...
export default function PurchaseLotList({ onViewDetail }: PurchaseLotListProps) {
    ...
    const fetchLots = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/bch/purchase/lots?status=${viewStatus}`, { cache: 'no-store' });
            if (!response.ok) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/bch/purchase/lots/${lotId}`, {
                method: 'DELETE',
            });
    ...
    const grandTotalQty = filteredLots.reduce((sum, lot) => sum + (Number(lot.totalQty) || 0), 0);
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', position: 'relative' }}>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div style={{
                        ...
                        Total Qty: {grandTotalQty.toLocaleString()}
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <button
                        onClick={() => router.push('/bch/purchase?section=purchase-lots-import')}
                        style={{
                            padding: '6px 16px',
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                    <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                        <tr>
                            ...
                            <th style={{ padding: '1rem', fontWeight: 600, color: '#475569', fontSize: '0.85rem' }}>Qty</th>
                    ...
                    <tbody>
                        {filteredLots.length > 0 ? filteredLots.map((lot) => (
                            <tr key={lot.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                ...
                                <td style={{ padding: '1rem', color: '#64748b', fontSize: '0.9rem' }}>{lot.totalQty || 0}</td>
