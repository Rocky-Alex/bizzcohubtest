...
export default function PurchaseLotDetails({ lotId, onBack }: PurchaseLotDetailsProps) {
    ...
    const handleSave = async (itemId: number) => {
        try {
            ...
            const res = await fetch(`/api/bch/purchase/lots/items/${itemId}`, {
                method: 'PUT',
    ...
    const handleDelete = async (itemId: number) => {
        ...
        try {
            const res = await fetch(`/api/bch/purchase/lots/items/${itemId}`, { method: 'DELETE' });
            if (res.ok) {
                                                            ...
                                                            <button onClick={async () => {
                                                                ...
                                                                try {
                                                                    ...
                                                                    await Promise.all(item.ids.map(id =>
                                                                        fetch(`/api/bch/purchase/lots/items/${id}`, {
                                                                            method: 'PUT',
                                                                            headers: { 'Content-Type': 'application/json' },
                                                                            body: JSON.stringify(payload)
                                                                        })
                                                        ...
                                                        <div style={{ display: 'flex', gap: '8px' }}>
                                                            ...
                                                            <button
                                                                onClick={async () => {
                                                                    ...
                                                                    try {
                                                                        await Promise.all(item.ids.map(id =>
                                                                            fetch(`/api/bch/purchase/lots/items/${id}`, { method: 'DELETE' })
                                                                        ));
                                                                        toast.dismiss(toastId);
