...
export default function InventoryQCChecking() {
    ...
    const fetchDropdownData = async () => {
        try {
            ...
            const res = await fetch(`/api/bch/inventory/droplists?category=${categoryList}`);
    ...
    const handleScan = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/bch/inventory/qc?sku=${encodeURIComponent(scanInput.trim())}`);
            const data = await res.json();
