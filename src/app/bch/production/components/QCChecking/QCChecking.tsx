...
export default function QCChecking() {
    ...
    const fetchInitialData = async () => {
        ...
        try {
            ...
            const [lotsRes, dropListsRes] = await Promise.all([
                fetch('/api/bch/purchase/lots?status=active', { cache: 'no-store' }),
                fetch(`/api/bch/inventory/droplists?category=${categoryList}`)
            ]);
    ...
    const fetchProductDetails = async (item: PurchaseLotItem) => {
        ...
        try {
            ...
            if (item.sku) {
                const res = await fetch(`/api/bch/inventory/products?sku=${item.sku}`);
                if (res.ok) {
            ...
            if (!product && item.productName) {
                ...
                const res = await fetch(`/api/bch/inventory/products?name=${encodedName}`);
                if (res.ok) {
                    product = await res.json();
                }
            }
