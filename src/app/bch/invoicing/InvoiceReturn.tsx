...
export default function InvoiceReturn({ setActiveSection }: InvoiceReturnProps) {
    ...
    const fetchInvoiceDetails = async (invoiceNo: string) => {
        ...
        try {
            ...
            if (!foundInvoice) {
                const res = await fetch(`/api/bch/invoices?invoiceNo=${invoiceNo.trim()}`);
                if (res.ok) {
                    const data = await res.json();
            ...
            if (foundInvoice) {
                ...
                const detailRes = await fetch(`/api/bch/invoices/${foundInvoice.id}`);
                if (detailRes.ok) {
    ...
    const handleProcessReturn = async () => {
        ...
        setConfirmModal({
            ...
            onConfirm: async () => {
                try {
                    const response = await fetch('/api/bch/invoices/return', {
                        method: 'POST',
