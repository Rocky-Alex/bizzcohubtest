...
export default function InvoiceList({ setActiveSection, onEdit }: { setActiveSection: (section: string) => void, onEdit: (invoice: any) => void }) {
    ...
    React.useEffect(() => {
        const fetchInvoices = async () => {
            try {
                const response = await fetch(`/api/bch/invoices?_t=${Date.now()}`, { cache: 'no-store' });
                if (response.ok) {
        ...
        window.addEventListener('dashboard-updated', fetchInvoices);

        return () => {
            clearInterval(interval);
            window.removeEventListener('dashboard-updated', fetchInvoices);
        };
    }, []);
    ...
    const handleStatusChange = (id: number, newStatus: string) => {
        setConfirmModal({
            ...
            onConfirm: async () => {
                ...
                try {
                    const response = await fetch(`/api/bch/invoices/${id}`, {
                        method: 'PATCH',
    ...
    const handleView = async (id: number) => {
        ...
        try {
            const res = await fetch(`/api/bch/invoices/${id}`, { cache: 'no-store' });
            if (res.ok) {
    ...
    const handleDelete = (id: number) => {
        setConfirmModal({
            ...
            onConfirm: async () => {
                ...
                try {
                    const res = await fetch(`/api/bch/invoices/${id}`, { method: 'DELETE' });
                    if (!res.ok) {
                        setConfirmModal({
                            isOpen: true,
                            title: 'Error',
    ...
    const handlePayment = async (inv: any) => {
        ...
        try {
            const res = await fetch(`/api/bch/invoices/${inv.id}/payments`);
            if (res.ok) {
                const data = await res.json();
    ...
    const submitPayment = async () => {
        ...
        const url = editingPaymentId
            ? `/api/bch/invoices/${selectedInvoiceId}/payments/${editingPaymentId}`
            : `/api/bch/invoices/${selectedInvoiceId}/payments`;
        ...
        try {
            ...
            if (res.ok) {
                ...
                const histRes = await fetch(`/api/bch/invoices/${selectedInvoiceId}/payments`);
                if (histRes.ok) {
    ...
    const handleDeletePayment = async (payId: number) => {
        ...
        setConfirmModal({
            ...
            onConfirm: async () => {
                setConfirmModal(prev => ({ ...prev, isOpen: false }));
                try {
                    const res = await fetch(`/api/bch/invoices/${selectedInvoiceId}/payments/${payId}`, {
                        method: 'DELETE'
                    });
                    const data = await res.json();
    ...
    const handleSendReceipt = async (payId: number) => {
        ...
        setConfirmModal({
            ...
            onConfirm: async () => {
                setConfirmModal(prev => ({ ...prev, isOpen: false }));
                try {
                    const res = await fetch(`/api/bch/invoices/${selectedInvoiceId}/payments/${payId}/send`, { method: 'POST' });
                    if (res.ok) {
                        setConfirmModal({
                            isOpen: true,
                            title: 'Success',
        ...
        setConfirmModal({
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    const res = await fetch(`/api/bch/invoices/${id}/send`, { method: 'POST' });
                    const data = await res.json();
