...
interface FourBySixEditorProps {
    settingKey?: string;
    title?: string;
}

export default function FourBySixEditor({ settingKey = 'shipping_4x6', title = '4x6 Label' }: FourBySixEditorProps) {
    const printRef = useRef<HTMLDivElement>(null);

    // Default connection to 'cm' (4x6 inches is approx 10.16 x 15.24 cm)
    // Detailed default positions for a vertical layout
    const [config, setConfig] = useState<LabelConfig>({
        ...
        productNameSize: 16,
        productNamePos: { x: 0.5, y: 1.0 },
        productNameWidth: 9,

        barcodeScale: 2,
        barcodeHeight: 60,
        barcodeFontSize: 14,
        barcodePos: { x: 0.5, y: 5.0 },

        specsFontSize: 12,
        specsLineHeight: 1.5,
        specsPos: { x: 0.5, y: 8.0 },
    ...
    React.useEffect(() => {
        const fetchSettings = async () => {
            try {
                const res = await fetch(`/api/bch/label-settings?name=${settingKey}`, { cache: 'no-store' });
    ...
    const handleSave = async () => {
        try {
            const res = await fetch('/api/bch/label-settings', {
                method: 'POST',
    ...
    return (
        <div className="four-by-six-editor" style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', justifyContent: 'center' }}>
