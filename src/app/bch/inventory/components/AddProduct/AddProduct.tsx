...
export default function AddProduct({ onCancel, onSuccess, initialData }: AddProductProps) {
    ...
    useEffect(() => {
        const fetchAllDropLists = async () => {
            try {
                const categories = 'Laptop,RAM,Storage,Graphics';
                const res = await fetch(`/api/bch/inventory/droplists?category=${categories}`);
                const data = await res.json();

                if (data.success && data.categoryData) {
                    setDropLists({
                        laptop: data.categoryData['Laptop'] || [],
                        ram: (data.categoryData['RAM'] || []).map((i: any) => i.value),
                        storage: (data.categoryData['Storage'] || []).map((i: any) => i.value),
                        graphics: (data.categoryData['Graphics'] || []).map((i: any) => i.value)
                    });
                }
            } catch (error) {
    ...
    const generateProductCode = React.useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/bch/inventory/products/next-code?prefix=${prefix}`);
            if (res.ok) {
                const data = await res.json();
                if (data.code) {
                    setFormData(prev => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch('/api/bch/inventory/products', {
                method,
