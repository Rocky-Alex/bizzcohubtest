...
export async function GET(): Promise<NextResponse> {
    try {
        ...
        const [overallStats, currentMonthStats, prevMonthStats, recentTransactions, monthlyRevenue, quotationStats] = await Promise.all([
            ...
            sql`
                SELECT id, quotation_no, customer_name, total_amount, status, created_date 
                FROM quotations 
                WHERE TRIM(status) != 'Cancelled'
        ...
        const result = {
            totalQuotations: Number(totals.total_quotations) || 0,
            ...
            trends: {
                quotations: calcTrend(Number(cm.count), Number(pm.count)),
