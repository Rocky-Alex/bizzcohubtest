...
export async function PATCH(req: Request, { params }: { params: { id: string, paymentId: string } }): Promise<NextResponse> {
    try {
        ...
        await sql`
            ...
            SET amount = ${newAmountVal}, payment_date = ${date}, payment_method = ${method}, notes = ${notes}, staff_name = ${staff_name || oldPayment.staff_name}
            WHERE id = ${paymentId}
        `;

        // 3. Update Quotation Totals
        // Fetch current quotation total paid
        const invResult = await sql`SELECT total_amount, advance_received FROM quotations WHERE id = ${quotationId}` as unknown as any[];
...
export async function DELETE(req: Request, { params }: { params: { id: string, paymentId: string } }): Promise<NextResponse> {
    try {
        ...
        // 2. Delete Payment
        await sql`DELETE FROM quotation_payments WHERE id = ${paymentId}`;
