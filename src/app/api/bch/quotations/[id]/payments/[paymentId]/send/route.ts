...
export async function POST(req: Request, context: any): Promise<NextResponse> {
    try {
        ...
        const paymentResult = await sql`SELECT * FROM quotation_payments WHERE id = ${paymentId}` as unknown as any[];

        if (quotationResult.length === 0 || paymentResult.length === 0) {
            return NextResponse.json({ error: 'Quotation or Payment not found' }, { status: 404 });
        }
        ...
        const subject = `Payment Receipt for Quotation #${quotation.quotation_no}`;
        const htmlContent = `
            <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 8px;">
                ...
                <div style="margin-bottom: 20px;">
                    <p style="margin: 5px 0; font-size: 14px;"><strong>Quotation Details:</strong></p>
                    <p style="margin: 2px 0; color: #666; font-size: 13px;">Quotation #: ${quotation.quotation_no}</p>
                    <p style="margin: 2px 0; color: #666; font-size: 13px;">Total Quotation Amount: $${Number(quotation.total_amount).toFixed(2)}</p>
                    <p style="margin: 2px 0; color: #666; font-size: 13px;">Total Paid So Far: $${Number(quotation.advance_received).toFixed(2)}</p>
                    <p style="margin: 2px 0; color: #dc2626; font-size: 13px;"><strong>Balance Due: $${(Number(quotation.total_amount) - Number(quotation.advance_received)).toFixed(2)}</strong></p>
                </div>
