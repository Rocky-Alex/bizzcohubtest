...
export default function ProductForm({
    ...
    const handleSubmit = async () => {
        ...
        if (!productCode) {
            try {
                // Fetch next sequential product code from API
                const response = await fetch(`/api/products/generate-code?type=${type}`);
                if (response.ok) {
                    ...
                    console.warn('Failed to generate sequential code, using random:', productCode);
                }
            } catch (error) {
                // Fallback to random if API fails
                const prefix = type === 'laptop' ? 'BCH-LP' : 'BCH-AC';
