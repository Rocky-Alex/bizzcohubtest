...
export default function ImportPurchaseLot({ onCancel, onSuccess }: ImportPurchaseLotProps) {
    ...
    const parseFile = (file: File) => {
        ...
        reader.onload = (e) => {
            ...
            let extractedDate = worksheet['B3']?.v;
            ...
            const jsonData: any[] = XLSX.utils.sheet_to_json(worksheet, { range: 5 });
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const items = previewData.map((row: any) => {
                // Construct product name from Brand + Series + Model + Processor
                const brand = row.Brand || '';
                const series = row.Series || '';
                const model = row.Model || '';
                const processor = row.Processor || '';
                const gen = row['Processor Gen'] || '';

                const constructedName = `${brand} ${series} ${model} ${processor} ${gen}`.trim().replace(/\s+/g, ' ');
                const productType = row['Category'] || row['Product Types'] || 'Product';
                ...
                return {
                    ...
                    processor: processor,
                    processorGen: gen,
                    productName: constructedName || row.product_name || row.item_name || row['Product Name'] || 'Unnamed Product',
                    sku: row.sku || row.product_code || row.SKU || '',
                    quantity: parseNumber(row.Qty || row.quantity || row.qty),
                    unitCost: parseNumber(row.unit_cost || row.cost || row['Unit Cost']),
            ...
            const response = await fetch('/api/admin/inventory/purchase-lots/bulk', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
