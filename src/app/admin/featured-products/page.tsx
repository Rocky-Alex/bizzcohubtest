...
export default function FeaturedProductsPage() {
    ...
    const handleClearSlot = (index: number) => {
        const newSlots = [...slots];
        newSlots[index] = { slot_number: index + 1, product_code: '' };
        setSlots(newSlots);
        handleSave(newSlots);
    };
    ...
    const breadcrumbs = ['Home', 'Featured Products'];
    const filledSlots = slots.filter(s => s.product_code && s.product_code.trim() !== '');

    return (
        <div style={{ padding: '0.5rem' }}>
            <div style={{ padding: '2rem', background: 'var(--bg-primary, white)', borderRadius: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', color: 'var(--text-primary)' }}>
                    ...
                    {slots.map((slot, idx) => (
                        <div key={slot.slot_number} style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                <label style={{ fontWeight: 500, fontSize: '0.9rem', color: 'var(--text-secondary, #555)' }}>Slot {slot.slot_number}</label>
                                {slot.product_code && (
                ...
                <div>
                    ...
                    {filledSlots.length === 0 ? (
                        <p style={{ color: 'var(--text-tertiary, #888)', fontStyle: 'italic' }}>No featured products configured yet.</p>
                    ) : (
                        <div style={{ overflowX: 'auto' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.95rem' }}>
                                <thead>
                                    <tr style={{ background: 'var(--bg-tertiary, #f9fafb)', borderBottom: '1px solid var(--border, #e5e7eb)', textAlign: 'left' }}>
                                ...
                                <tbody>
                                    {filledSlots.map((slot) => (
                                        <tr key={slot.slot_number} style={{ borderBottom: '1px solid var(--border-light, #f3f4f6)' }}>
                                            <td style={{ padding: '12px', fontWeight: 600 }}>#{slot.slot_number}</td>
                                            <td style={{ padding: '12px' }}>
                                                {slot.primary_image_url ? (
                                                    <img src={slot.primary_image_url} alt="Product" style={{ width: '40px', height: '40px', objectFit: 'contain', borderRadius: '4px', border: '1px solid var(--border)' }} />
                                                ) : (
                                                    <div style={{ width: '40px', height: '40px', background: 'var(--bg-tertiary)', borderRadius: '4px' }}></div>
                                                )}
                                            </td>
                                            <td style={{ padding: '12px' }}>
                                                <div style={{ fontWeight: 500 }}>{slot.product_name || <span style={{ color: 'orange' }}>Product not found (check code)</span>}</div>
                                            </td>
                                            <td style={{ padding: '12px', color: 'var(--text-secondary)' }}>{slot.product_code}</td>
