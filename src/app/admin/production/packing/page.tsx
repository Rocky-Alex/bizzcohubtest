...
const BOX_TYPES = ['Small Box', 'Medium Box', 'Large Box', 'Crate', 'Pallet', 'Custom'];
...
export default function PackingPage() {
    ...
    const [boxType, setBoxType] = useState<string>(BOX_TYPES[1]);
    ...
    const fetchPackingHistory = async (orderId: string) => {
        try {
            const res = await fetch(`/api/admin/packing/${orderId}`);
            if (res.ok) {
                const data = await res.json();
    ...
    const handleBarcodeSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/admin/inventory/qc?sku=${encodeURIComponent(code)}`);
    ...
    const handleDeleteBox = async (boxId: number, isLocal?: boolean) => {
        ...
        try {
            const res = await fetch(`/api/admin/packing/boxes/${boxId}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    const handleFinalSubmit = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/admin/packing/create', {
                method: 'POST',
                    ...
                    <div style={{ background: 'white', padding: '1.5rem', borderRadius: '16px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                                ...
                                <select
                                    style={{
                                        gridColumn: 'span 2', padding: '0.5rem', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '0.8rem', outline: 'none'
                                    }}
                                    value={BOX_TYPES.includes(boxType) && BOX_TYPES.indexOf(boxType) >= 4 ? boxType : 'More...'}
                                    onChange={(e) => setBoxType(e.target.value)}
                                >
                                    <option disabled>More...</option>
                                    {BOX_TYPES.slice(4).map(type => <option key={type} value={type}>{type}</option>)}
                                </select>
                            </div>
                        </div>
                    </div>
                                ...
                                <form onSubmit={handleBarcodeSubmit} style={{ position: 'relative' }}>
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            fontWeight: 600,
                                            fontSize: '0.85rem',
                                            opacity: isScanning || !selectedOrderId ? 0.6 : 1,
                                            transition: 'all 0.2s'
                                        }}
                                        disabled={isScanning || !selectedOrderId}
                                    >
                                        Add
                                    </button>
                                </form>
