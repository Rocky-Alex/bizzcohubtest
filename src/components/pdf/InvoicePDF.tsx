...
const InvoicePDF: React.FC<InvoicePDFProps> = ({ invoice, items, logoUrl, staffName, staffRole }) => {
    ...
    const taxRate = Number(invoice.tax_rate || 5);
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.infoSection}>
                    <View style={styles.billToColumn}>
                        ...
                        <Text style={{ fontWeight: 'bold', fontSize: 10, marginBottom: 2, color: '#000' }}>{invoice.customer_name}</Text>
                        <Text style={styles.billToContent}>{invoice.customer_address}</Text>
                        {invoice.customer_email && <Text style={styles.billToContent}>{invoice.customer_email}</Text>}
                        {invoice.customer_phone && <Text style={styles.billToContent}>{invoice.customer_phone}</Text>}
                    </View>
                ...
                <View style={styles.table}>
                    ...
                    {items.map((item, index) => (
                        <View style={styles.tableRow} key={index}>
                            ...
                            <Text style={[styles.tableCell, styles.colCost, { color: '#333' }]}>AED {Number(item.unit_price).toFixed(0)}</Text>
                            <Text style={[styles.tableCell, styles.colDisc, { color: '#333' }]}>AED {Number(item.discount || 0).toFixed(0)}</Text>
                            <Text style={[styles.tableCell, styles.colTotal, { color: '#333' }]}>AED {Number(item.total).toFixed(0)}</Text>
                        </View>
                    ))}
                </View>
                ...
                <View style={styles.bottomSection}>
                    <View style={styles.termsColumn}>
                        {invoice.show_terms !== false && (
                            <>
                                <Text style={styles.sectionTitle}>Terms and Conditions</Text>
                                <Text style={styles.sectionContent}>{invoice.terms_and_conditions || ''}</Text>
                    ...
                    <View style={styles.totalsColumn}>
                        {(invoice.is_taxable || invoice.is_discountable) && (
                        ...
                        <View style={styles.totalRow}>
                            ...
                            <Text style={[styles.totalValue, { color: '#16a34a', fontWeight: 'bold' }]}>AED {advancePaid.toFixed(0)}</Text>
                        </View>
